  ...
  return (
    <div className="min-h-[calc(100svh-9rem)] px-4 py-8">
      <form onSubmit={submit} className="mx-auto max-w-[520px] rounded-3xl bg-white p-6 text-slate-900">
        <h1 className="text-3xl font-bold">বাংলা বার্তা তৈরি করুন</h1>
            ...
            <div className="mt-3 grid grid-cols-2 gap-3">
              {tones.map((item) => (
                <button
                  ...
                  className={`min-h-12 rounded-2xl border px-4 font-semibold focus:outline focus:outline-2 focus:outline-rose-500 ${tone === item.value ? "border-rose-500 bg-rose-50 text-rose-700" : "border-slate-300 text-slate-800"}`}
                >
                  {item.label}
                </button>
              ))}
